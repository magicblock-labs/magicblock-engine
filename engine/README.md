...
rolled back. Successful finalization marks delegated, ephemeral, and transient
accounts authoritative, preventing later MagicRoot replacement. `create` may
still run its `PostFinalize` actions immediately after that finalization in the
same transaction. Magicblock construction rejects instruction, address,
account-meta, and instruction-data lengths that cannot be represented by the V1
wire fields.

## Startup and recovery

Keeper restores an accountsdb snapshot when the active store is corrupt or its
sealed superblock trails the retained ledger. The latter also covers snapshots
