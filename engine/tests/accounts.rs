...
use solana_account::{
    AccountBuilder, AccountFieldPatch, AccountMode, OwnedAccount, ReadableAccount, StateFlags,
};
...
// field then finalizing it (finalize drains the created balance from the
// authority, conserving supply); `patch` mutates individual fields in place;
// `update` overwrites an existing account or materializes a fresh key; and
// `delete` closes it. `patch`/`update` on an already-funded account must not
// change lamports — finalize would re-charge the authority for the delta and
// unbalance the transaction — so mutations here keep the balance constant.
...
async fn account_crud_lifecycle() {
    ...
    // patch sequence extends but never truncates) and identical lamports, so
    // finalize's delta charge is zero and the transaction stays balanced. The
    // owner reverting to `owner` proves the earlier patch was overwritten.
...
// the exact data length. The caller leaves these accounts unprotected, so each
// later replacement remains permitted.
#[tokio::test(flavor = "multi_thread")]
...
async fn account_update_replaces_v42_program_at_new_slot() {
    ...
    let replacement = AccountBuilder::from(program).slot(slot).build();
...
async fn create_runs_post_finalize_actions() {
    ...
    let acc = v42_builder(0, AccountMode::Delegated).build();
