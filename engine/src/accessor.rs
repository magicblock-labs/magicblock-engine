...
impl AccountAccessor<'_> {
    ...
    pub async fn create(
        ...
        let mut instructions = MagicRootInstruction::compose_account(self.pubkey, account)?;
    ...
    pub async fn update(&self, account: OwnedAccount) -> Result<()> {
        let instructions = MagicRootInstruction::compose_account(self.pubkey, account)?;
        self.execute(instructions).await
    }
