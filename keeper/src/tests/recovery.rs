...
async fn seeds_features_programs_and_sysvars() {
    ...
    for (&id, &slot) in keeper.features().active() {
        ...
        let acc = keeper.accounts().get(&id).unwrap().expect("feature account seeded");
    ...
    let acc = keeper.accounts().get(&program).unwrap().expect("program seeded");
    ...
    for id in [Rent::id(), EpochSchedule::id()] {
        let acc = keeper.accounts().get(&id).unwrap().expect("sysvar seeded");
...
async fn recovers_the_newest_snapshot() {
    ...
    let restored = keeper.accounts().get(&marker).unwrap().expect("marker restored");
