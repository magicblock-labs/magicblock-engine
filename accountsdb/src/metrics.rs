...
use nucleus::metrics::{
    IntCounter, IntCounterVec, IntGaugeVec, MetricOperation, MetricSpec, OperationCounters,
};
...
pub(crate) fn accounts(store: StoreKind, count: u64) {
    ...
    metric::with_metrics(&METRICS, |m| {
        m.accounts.with_label_values(&[store.label()]).set(count);
    });
}
...
struct Metrics {
    ...
    resizes: IntCounter,
    /// Registered load counter labeled by source or absence; backs `loads`.
    loads_vec: IntCounterVec,
...
impl Metrics {
    ...
    fn register(&self) {
        metric::register("accountsdb", self.reads.clone());
        metric::register("accountsdb", self.commits.clone());
        metric::register("accountsdb", self.allocs.clone());
        metric::register("accountsdb", self.reallocs.clone());
        metric::register("accountsdb", self.compactions.clone());
        metric::register("accountsdb", self.removals.clone());
        metric::register("accountsdb", self.resizes.clone());
        metric::register("accountsdb", self.loads_vec.clone());
        self.operations.register("accountsdb");
        metric::register("accountsdb", self.accounts.clone());
    }
}
