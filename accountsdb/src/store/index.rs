...
/// Read-only transaction utilizing thread-local storage.
...
/// Optional read transaction used by batched loads.
...
impl Index {
    ...
    pub(crate) fn allocate(&self, units: u32, txn: &mut RwTxn<'_>) -> Result<Option<Offset>> {
        ...
        if offset.is_some() {
            self.freelist.delete(txn, &units)?;
        }
        Ok(offset)
    }
    ...
    pub(crate) fn program<'a>(&'a self, owner: Pubkey) -> Result<Option<OwnerIter<'a>>> {
        ...
        let iter = unsafe { mem::transmute::<_, RoProgramIter<'_>>(iter) };
    ...
    pub(crate) fn unwrap_rw<'t, 'e>(&'e self, txn: OptRwTxn<'t, 'e>) -> Result<&'t mut RwTxn<'e>> {
        if let Some(txn) = txn {
            return Ok(txn);
        }
        Ok(txn.insert(self.env.write_txn()?))
    }

    /// Uses the supplied read transaction or opens one on demand.
    pub(crate) fn unwrap_ro<'t, 'e>(&'e self, txn: OptRoTxn<'t, 'e>) -> Result<&'t RoTxnTls<'e>> {
        if let Some(txn) = txn {
            return Ok(txn);
        }
        Ok(txn.insert(self.env.read_txn()?))
    }

    /// Flush the index databases onto the durable storage (sync)
