...
impl TransactionAccounts {
    ...
    pub fn try_borrow_mut(
        ...
        let keyed_account = unsafe { &mut *self.accounts.get(index as usize).unwrap().get() };

        let account = TransactionAccountViewMut { account: &mut keyed_account.1 };
...
mod tests {
    ...
    fn preserves_account_shared_data_on_deconstruct() {
        ...
        assert!(account.flags().contains(StateFlags::PROTECTED));
