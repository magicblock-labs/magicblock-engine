...
contains `EXECUTABLE` and `PROTECTED`; callers set the latter when MagicRoot
must not replace or delete the supplied state. MagicRoot does not infer the flag
from `AccountMode` or set it during finalization.
