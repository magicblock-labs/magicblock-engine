use {
    crate::{
        bytes::{
            ...
            try_u32_offset, unchecked_copy_value, unchecked_read_byte, unchecked_read_slice_data,
        },
...
impl InstructionsFrame {
    ...
    pub(crate) fn try_new_for_legacy_and_v0(bytes: &[u8], offset: &mut usize) -> Result<Self> {
        ...
        let minimum_instructions_len = 3usize
            .checked_mul(usize::from(num_instructions))
            .ok_or(TransactionViewError::ParseError)?;
        check_remaining(bytes, *offset, minimum_instructions_len)?;

        let instructions_offset = try_u32_offset(*offset)?;
        ...
        for _index in 0..num_instructions {
            ...
            let num_accounts_len = u8::try_from(
                offset
                    .checked_sub(num_accounts_offset)
                    .ok_or(TransactionViewError::ParseError)?,
            )
            ...
            let data_len_len = u8::try_from(
                offset.checked_sub(data_len_offset).ok_or(TransactionViewError::ParseError)?,
            )
            .map_err(|_| TransactionViewError::ParseError)?;
    ...
    pub(crate) fn try_new_for_v1(
        ...
        let headers_offset = try_u32_offset(*offset)?;
        let headers_len = core::mem::size_of::<V1InstructionHeader>()
            .checked_mul(usize::from(num_instructions))
            .ok_or(TransactionViewError::ParseError)?;
        ...
        *offset = offset.checked_add(headers_len).ok_or(TransactionViewError::ParseError)?;

        let payloads_offset = try_u32_offset(*offset)?;
        ...
        *offset = offset.checked_add(total_payload_len).ok_or(TransactionViewError::ParseError)?;
    ...
    unsafe fn read_v1_header(bytes: &[u8], offset: &mut usize) -> V1InstructionHeader {
        ...
        *offset += core::mem::size_of::<V1InstructionHeader>();
...
unsafe fn for_legacy_and_v0<'a>(
    ...
    *offset += usize::from(num_accounts_len);
    ...
    // Move offset to accounts offset - do not re-parse u16.
    *offset += usize::from(data_len_len);
    const _: () = assert!(core::mem::align_of::<u8>() == 1, "u8 alignment");
    // SAFETY:
    // - The offset is checked to be valid in the byte slice.
    // - The alignment of u8 is 1.
