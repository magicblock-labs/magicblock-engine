use {
    crate::{
        bytes::{
            ...
            read_compressed_u16, read_slice_data, read_type, try_u32_offset,
        },
...
impl AddressTableLookupFrame {
    ...
    pub(crate) fn try_new(bytes: &[u8], offset: &mut usize) -> Result<Self> {
        ...
        check_remaining(
            ...
            *offset,
            MIN_SIZED_ATL
                .checked_mul(usize::from(num_address_table_lookups))
                .ok_or(TransactionViewError::ParseError)?,
        )?;

        let address_table_lookups_offset = try_u32_offset(*offset)?;
        ...
        for _index in 0..num_address_table_lookups {
            ...
            total_writable_lookup_accounts = total_writable_lookup_accounts
                .checked_add(u32::from(num_write_accounts))
                .ok_or(TransactionViewError::ParseError)?;
            ...
            total_readonly_lookup_accounts = total_readonly_lookup_accounts
                .checked_add(u32::from(num_read_accounts))
                .ok_or(TransactionViewError::ParseError)?;
