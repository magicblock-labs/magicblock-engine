use crate::{
    bytes::{advance_offset_for_array, try_u32_offset, unchecked_copy_value},
...
impl TransactionConfigFrame {
    ...
    fn word_offset(&self, bit: u8) -> Option<usize> {
        ...
        (self.values_offset as usize).checked_add(word_index.checked_mul(Self::CONFIG_VALUE_SIZE)?)
    }
}
