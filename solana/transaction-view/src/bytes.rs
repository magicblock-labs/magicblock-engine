...
#[inline(always)]
pub(crate) fn try_u32_offset(offset: usize) -> Result<u32> {
    u32::try_from(offset).map_err(|_| TransactionViewError::ParseError)
}
...
pub fn check_remaining(bytes: &[u8], offset: usize, num_bytes: usize) -> Result<()> {
    let end = offset.checked_add(num_bytes).ok_or(TransactionViewError::ParseError)?;
    (end <= bytes.len()).then_some(()).ok_or(TransactionViewError::ParseError)
}
...
pub fn advance_offset_for_array<T: Sized>(
    ...
    let array_len_bytes = usize::from(num_elements)
        .checked_mul(core::mem::size_of::<T>())
        .ok_or(TransactionViewError::ParseError)?;
    check_remaining(bytes, *offset, array_len_bytes)?;
    *offset = offset.checked_add(array_len_bytes).ok_or(TransactionViewError::ParseError)?;
    Ok(())
}
...
pub fn advance_offset_for_type<T: Sized>(bytes: &[u8], offset: &mut usize) -> Result<()> {
    ...
    *offset = offset.checked_add(type_size).ok_or(TransactionViewError::ParseError)?;
    Ok(())
}
...
pub unsafe fn read_slice_data<'a, T: Sized>(
    ...
    let current_ptr = unsafe { bytes.as_ptr().add(start) };
...
pub unsafe fn unchecked_read_slice_data<'a, T: Sized>(
    ...
    let array_len_bytes = usize::from(num_elements) * core::mem::size_of::<T>();
