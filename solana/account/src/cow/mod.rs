...
pub struct AccountCore {
    ...
    /// Independent state modifier flags such as executable or authoritative.
...
impl AccountSharedData {
    ...
    pub(crate) fn set_flag(&mut self, flag: StateFlags, val: bool) {
        if self.flags.contains(flag) && val {
            return;
        }
        self.translate();
...
bitflags::bitflags! {
    ...
    pub struct StateFlags: u8 {
        ...
        /// Caller-declared account whose state MagicRoot must not mutate.
        const PROTECTED = 1 << 1;
    }
