...
impl AccountHeader {
    ...
    pub(crate) fn new(space: u32) -> Self {
        // `space` stays in StorageUnit's so the active
        // image can be indexed with one multiply.
        Self { sequence: 0.into(), space }
    }
}

/// IMPORTANT: Pointer arithmetic relies on these invariants (DO NOT CHANGE THEM!)
...
impl BorrowedAccount {
    ...
    /// buffer whose first bytes are an [`AccountHeader`], followed by two
    /// image-sized payloads. The active image is selected from `sequence`
    /// parity.
    ...
    /// Restores the active image after a failed write path.
    ///
    /// # Safety
    ///
    /// Call this only after `translate`; it only repoints the view back to the active image.
    pub unsafe fn reset(&mut self) {
        // Repoint to the previous active image; no bytes are copied back.
        let offset = offset(self.header.as_ref(), true);

        // Switch the pointers back to the active view
