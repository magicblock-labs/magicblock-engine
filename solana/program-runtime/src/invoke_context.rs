...
pub use crate::memory_context::SerializedAccountMetadata;

pub type BuiltinFunctionWithContext = (
    BuiltinFunction<InvokeContext<'static, 'static>>,
...
impl ContextObject for InvokeContext<'_, '_> {
    ...
    fn active_mapping_ptr(&mut self) -> NonNull<MemoryMapping> {
        let memory_mapping = self
            .memory_contexts
            .memory_mapping_mut()
            .expect("memory context must be set for the current instruction");
        NonNull::from(memory_mapping)
    }
}
...
impl<'a, 'ix_data> InvokeContext<'a, 'ix_data> {
    ...
    pub fn native_invoke_signed(
        ...
        let caller_program_id =
            *self.transaction_context.get_current_instruction_context()?.get_program_key()?;
    ...
    pub fn prepare_next_cpi_instruction(
        ...
        let program_account_index = {
            ...
            for account_meta in instruction.accounts.iter() {
                ...
                let index_in_callee =
                    transaction_callee_map.get_mut(index_in_transaction as usize).unwrap();
            ...
            let program_account_index_in_transaction =
                self.transaction_context.find_index_of_account(callee_program_id);
    ...
    fn process_executable_chain(
        ...
        let cache_id = if native_loader::check_id(&owner_id)
        ...
        let result = match &entry.program {
            ProgramCacheEntryType::Builtin(program) => {
                ...
                self.memory_contexts.set_memory_context_abi_v1(MemoryContext::new(
                    BpfAllocator::new(0),
                    Vec::new(),
                    // Built-ins do not dereference VM memory, but the invoke
                    // context still requires an active mapping.
                    unsafe {
            ...
            ProgramCacheEntryType::Loaded(program) => {
                ...
                let logger = self.get_log_collector();
                stable_log::program_invoke(&logger, &program_id, self.get_stack_height());
                let result = crate::vm::execute(program, self).map_err(|error| {
                    error
                ...
                match &result {
                    ...
                    Err(error) => stable_log::program_failure(&logger, &program_id, error),
                }
                result
            }
            _ => Err(InstructionError::UnsupportedProgramId),
        };
        ...
        sysvar_cache.fill_missing_entries(|pubkey, callback| {
            for index in 0..$transaction_context.get_number_of_accounts() {
                if $transaction_context.get_key_of_account_at_index(index).unwrap() == pubkey {
                    callback($transaction_context.accounts().try_borrow(index).unwrap().data());
                }
            }
        });
...
>(
    ...
    program_cache_for_tx_batch.replenish(
        *loader_id,
        Arc::new(ProgramCacheEntry::new_builtin(builtin_function)),
    );
    ...
    invoke_context
        ...
        .unwrap();
    let result = invoke_context.process_instruction(&mut 0);
...
mod tests {
    ...
    declare_process_instruction!(
        ...
        |invoke_context| {
            ...
            assert_eq!(
                program_id,
                instruction_context.try_borrow_instruction_account(0)?.get_owner()
            );
            assert_ne!(
                instruction_context.try_borrow_instruction_account(1)?.get_owner(),
    ...
    fn test_process_instruction_compute_unit_consumption(
        ...
        invoke_context.prepare_next_cpi_instruction(inner_instruction, &[]).unwrap();

        let mut compute_units_consumed = 0;
        let result = invoke_context.process_instruction(&mut compute_units_consumed);
    ...
    fn test_prepare_instruction_maximum_accounts() {
        ...
        for i in 2..MAX_ACCOUNTS_REFERENCED {
            ...
            if i < MAX_ACCOUNTS_PER_TRANSACTION {
                ...
                let repeated_key =
                    transaction_accounts.get(i % MAX_ACCOUNTS_PER_TRANSACTION).unwrap().0;
