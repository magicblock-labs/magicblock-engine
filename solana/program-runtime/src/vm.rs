...
/// Calculates extra compute units charged for a requested heap size.
...
/// Creates an SBF VM bound to the current invocation context.
#[cfg_attr(feature = "svm-internal", qualifiers(pub))]
pub fn create_vm<'a, 'b, 'c>(
    program: &'a Executable<InvokeContext<'b, 'c>>,
    regions: Vec<MemoryRegion>,
    accounts_metadata: Vec<SerializedAccountMetadata>,
    invoke_context: &'a mut InvokeContext<'b, 'c>,
    stack: &mut [u8],
    heap: &mut [u8],
) -> Result<EbpfVm<'a, InvokeContext<'b, 'c>>, Box<dyn std::error::Error>> {
    ...
    invoke_context.memory_contexts.set_memory_context_abi_v1(MemoryContext::new(
...
fn create_memory_mapping<'a, C: ContextObject>(
    ...
    let regions: Vec<MemoryRegion> = vec![
        ...
        MemoryRegion::new(&raw mut heap[..], MM_HEAP_START),
    ]
    .into_iter()
    ...
    ($vm:ident, $program:expr, $regions:expr, $accounts_metadata:expr, $invoke_context:expr $(,)?) => {
        ...
        let $vm = heap_cost_result.and_then(|_| {
            ...
            let vm = $crate::__private::create_vm(
                ...
                heap.as_slice_mut().get_mut(..heap_size as usize).expect("invalid heap size"),
            );
...
pub fn execute<'a, 'b, 'c>(
    ...
    let executable = unsafe {
        mem::transmute::<
            ...
            &'a Executable<InvokeContext<'b, 'c>>,
        >(executable)
    };
    ...
    let direct_account_pointers_in_program_input =
        invoke_context.get_feature_set().direct_account_pointers_in_program_input;
    ...
    let account_region_addrs = accounts_metadata
        .iter()
        .map(|m| {
            let vm_end = m.vm_data_addr.saturating_add(m.original_data_len as u64).saturating_add(
                if !is_loader_deprecated { MAX_PERMITTED_DATA_INCREASE as u64 } else { 0 },
            );
    ...
    let execution_result = {
        ...
        match result {
            ...
            ProgramResult::Err(mut error) => {
                ...
                if invoke_context.get_feature_set().deplete_cu_meter_on_vm_failure
                ...
                if let EbpfError::AccessViolation(access_type, vm_addr, len, _section_name) = error
                {
                    // Account data is directly mapped. A write to readonly data
                    // or an access into the reserved growth range appears as a
                    // memory violation; map it to the account-specific error.
                    if let Some((instruction_account_index, vm_addr_range)) = account_region_addrs
                        .iter()
                        .enumerate()
                        .find(|(_, vm_addr_range)| vm_addr_range.contains(&vm_addr))
                    {
                        ...
                        if vm_addr.saturating_add(len) <= vm_addr_range.end {
                            // The access was within the range of the account address space,
                            // but it might not be within the range of the actual data.
                            let is_access_outside_of_data =
                                vm_addr.saturating_add(len).saturating_sub(vm_addr_range.start)
                                    as usize
                ...
                Err(if let EbpfError::SyscallError(err) = error { err } else { error.into() })
            }
            _ => Ok(()),
        }
    };
    ...
    fn deserialize_parameters(
        ...
        serialization::deserialize_parameters(
            &invoke_context.transaction_context.get_current_instruction_context()?,
            parameter_bytes,
