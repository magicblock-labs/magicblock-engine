...
async fn streams_and_resumes_without_duplicate_application() {
    ...
    let authority_before =
        leader.get_account(authority).expect("leader sponsor exists").lamports();
    let sponsored = Pubkey::new_unique();
    leader
        .account(sponsored)
        .create(v42_builder(0, AccountMode::Delegated).build(), None)
        .await
        .expect("sponsored account creation commits");
    let authority_after =
        leader.get_account(authority).expect("leader sponsor remains").lamports();
    assert!(
